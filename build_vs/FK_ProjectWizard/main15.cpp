...
bool addEnvPath(string &dirPath)
{
	...
	if(result == ERROR_SUCCESS) {
		if(RegSetValueEx(hkey, "FK_VC15_DIR", NULL, dataType, (LPBYTE)tmpStr, dataSize) == ERROR_SUCCESS) {
			RegCloseKey(hkey);
...
bool deleteEnvPath(void)
{
	...
	result = RegOpenKeyEx(HKEY_CURRENT_USER, "Environment", 0, KEY_WRITE, &hkey);
	if(result == ERROR_SUCCESS) {
		if(RegDeleteValue(hkey, "FK_VC15_DIR") == ERROR_SUCCESS) {
			RegCloseKey(hkey);
...
{
	// x64環境の製品版を確認
	if(readRegValue("SOFTWARE\\Wow6432Node\\Microsoft\\VisualStudio\\14.0\\Setup\\VC", instPath)) {
		return 1;
	}
	// 製品版を確認
	if(readRegValue("SOFTWARE\\Microsoft\\VisualStudio\\14.0\\Setup\\VC", instPath)) {
		return 1;
	}
	// x64環境のExpressを確認
	if(readRegValue("SOFTWARE\\Wow6432Node\\Microsoft\\VCExpress\\14.0\\Setup\\VC", instPath)) {
		return 2;
	}
	// Expressを確認
	if(readRegValue("SOFTWARE\\Microsoft\\VCExpress\\14.0\\Setup\\VC", instPath)) {
		return 2;
	}

	MessageBox(NULL, "Visual Studio 2015 がインストールされていませんでした。", "ウィザード登録中止", NULL);
	return 0;
}
	...
	if(argc >= 2) {
		if(strcmp("-u", argv[1]) == 0) {
			...
			if(unregist(vcprojPath, vcwizPath)) {
				MessageBox(NULL, "Visual Studio 2015 からプロジェクトウィザードを削除しました。", "ウィザード削除成功", NULL);
				return 0;
			} else {
				MessageBox(NULL, "Visual Studio 2015 からのウィザード削除に失敗しました。\n既に削除済みだったか、一部のファイルが残っている可能性があります。", "ウィザード削除失敗", NULL);
				return 1;
			}
		}
	} else {
		...
		if(regist(vcprojPath, vcwizPath)) {
			MessageBox(NULL, "Visual Studio 2015 にプロジェクトウィザードを登録しました。", "ウィザード登録成功", NULL);
		} else {
			unregist(vcprojPath, vcwizPath);
			MessageBox(NULL, "Visual Studio 2015 へのウィザード登録に失敗しました。\nプロジェクトの作成は手動で行ってください。", "ウィザード登録失敗", NULL);
			return 1;
		}
	}
