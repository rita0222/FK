...
\begin{verbatim}
    // 後ろを向く
    model.loAngle(fk_Math::PI, 0.0, 0.0);              

    // 右を向く
    model.loAngle(fk_Math::PI/2.0, 0.0, 0.0);          

    // 左を向く
    model.loAngle(-fk_Math::PI/2.0, 0.0, 0.0);

    // 上を向く
    model.loAngle(0.0, fk_Math::PI/2.0, 0.0);

    // 下を向く
    model.loAngle(0.0, -fk_Math::PI/2.0, 0.0);

    // 右上を向く
    model.loAngle(fk_Math::PI/2.0, fk_Math::PI/4.0, 0.0);

    // 左前上方を向く
    model.loAngle(-fk_Math::PI/4.0, fk_Math::PI/4.0, 0.0);

    // わずかに右を向く
    model.loAngle(fk_Math::PI/100.0, 0.0, 0.0);

    // わずかに上を向く
    model.loAngle(0.0, fk_Math::PI/100.0, 0.0);

    // 視線を軸に半回転。
    model.loAngle(0.0, 0.0, fk_Math::PI);

    // モデルを右に傾ける。
    model.loAngle(0.0, 0.0, fk_Math::PI/2.0);

    // わずかに右に傾ける。
    model.loAngle(0.0, 0.0, fk_Math::PI/100.0);
\end{verbatim}
\end{breakbox}
~ \\
...
\begin{verbatim}
        ...
        fk_Vector pos(0.0, 0.0, 0.0);
                :
                :
        model.glRotate(pos, fk_X, fk_Math::PI/4.0);
\end{verbatim}
\end{breakbox}
~ \\
